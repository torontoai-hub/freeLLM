...
async def apply_rate_limit(token: TokenConfig, skip: bool = False) -> RateLimitResult | None:
    if skip:
        return None
...
def set_rate_headers(response: Response, result: RateLimitResult | None) -> None:
    if result is None:
        return
...
@app.api_route("/internal/authz", methods=["GET", "POST"], include_in_schema=False)
async def authz(request: Request) -> Response:
    token = await auth.require_token(request)
    model_name = ""
    ...
    settings = state.settings or get_settings()
    backend_name, _ = resolve_backend(model_name) if model_name else (settings.default_backend, "")
...
async def chat_completions(
    ...
    rate_info = await apply_rate_limit(token, skip=request.headers.get("x-rate-limit-checked") == "1")
